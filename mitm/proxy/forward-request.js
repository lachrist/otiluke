...
module.exports = (intercept, {host, hostname, port, prefix, emitter}) => (request1, response1) => {
  if (request1.url.startsWith(prefix)) {
    ...
    request2.on("response", (response2) => {
      ...
      if (!closure) {
        ...
        response2.on("end", () => {
          const buffer = Buffer.from(closure(Buffer.concat(buffers, length).toString("utf8"), "http://"+hostname+":"+port+request1.url), "utf8");
